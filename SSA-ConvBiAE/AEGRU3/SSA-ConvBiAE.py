...
def fit_lstm(train, n_lag, n_seq, n_batch, nb_epoch, n_neurons):
    ...
    # model.add(ConvLSTM2D(filters=64, kernel_size=(1, 2), return_sequences=False))
    # model.add(Activation('tanh'))
    # model.add(Dropout(0.1))
    model.add(Flatten())
    model.add(RepeatVector(n_seq))
    # model.add(GRU(100, stateful=False, return_sequences=True))

    model.add(Bidirectional(GRU(n_neurons, stateful=False, return_sequences=False)))
    model.add(Activation('tanh'))
    model.add(Dropout(0.1))
